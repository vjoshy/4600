...
dense_mu  = Layer_Dense(128,latent_dim, weight_regularizer_l2=5e-4,bias_regularizer_l2=5e-4)
dense_logvar = Layer_Dense(128,latent_dim, weight_regularizer_l2=5e-4,bias_regularizer_l2=5e-4)


# decoder
dense3 = Layer_Dense(latent_dim,128, weight_regularizer_l2=5e-4,bias_regularizer_l2=5e-4)
...
for ep in range(episodes):
    ...
    regularization_loss = mse_loss.reg_loss(dense1) + mse_loss.reg_loss(dense_mu) + mse_loss.reg_loss(dense_logvar) + mse_loss.reg_loss(dense3) + mse_loss.reg_loss(dense4)

    loss = data_loss + regularization_loss +  (kl_loss)
    ...
    # Gradient for KL loss with respect to mu and logvar
    dmu = mu  # Gradient of KL loss with respect to mu
    dlogvar = 0.5 * (np.exp(logvar) - 1)  # Gradient of KL loss with respect to logvar

    # Combine gradients at the sampling point (z)
    dz = dense3.dinputs

    # Backpropagate through reparameterization trick

    dmu += dz
    dlogvar += dz * eps * np.exp(0.5 * logvar) # Same eps used in forward pass

    # Backward pass through mu and logvar layers
